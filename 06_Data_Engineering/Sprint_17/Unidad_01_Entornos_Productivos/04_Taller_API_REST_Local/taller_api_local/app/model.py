...
import json
import io # para que vaya más rápido
...
MODELS_DIR.mkdir(parents=True, exist_ok=True) # mkdir : me crea las rutas de lo anterior si no existen
    ...
    if csv_bytes is not None:
        ...
        if not DATA_PATH.exists():
            raise FileNotFoundError(
                f'No se encontro el dataset {DATA_PATH}. Sube un CSV o colócalo en esta ruta.'
            )
...
def train_model(csv_bytes:bytes | None = None, test_size:float = 0.2, random_state: int = 42) -> dict:
    ...
    metadata = {
        ...
        'intecept': round(model.intercept_, 4)
    }
