...
async def train(file: UploadFile | None = File(default=None)): #es para que no responda la app hasta que cumpla la condición como un 'if'
    """
    Reentrena el modelo.
...
def predict(payload= PredictResponse):
    try:
        pred, version = ml.predict(payload.TV, payload.Raio, payload.Newspaper)
